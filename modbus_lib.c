...
void modbus_lib_end_of_telegram(){
    ...
    switch (g_modbus_lib_received_telegram[1]){
        ...
        case MB_FUNC_WRITE_MULTIPLE_REGISTERS:
            ...
            outgoing_telegram[oindex++] = g_modbus_lib_received_telegram[1];    // function code
            outgoing_telegram[oindex++] = g_modbus_lib_received_telegram[2];
            outgoing_telegram[oindex++] = g_modbus_lib_received_telegram[3];
            outgoing_telegram[oindex++] = g_modbus_lib_received_telegram[4];
            outgoing_telegram[oindex++] = g_modbus_lib_received_telegram[5];                    // byte count

            for (uint16_t i=0; i < count.value; i++)
            {
				value.bytes.high = g_modbus_lib_received_telegram[7+2*i];
            	value.bytes.low = g_modbus_lib_received_telegram[8+2*i];
            	errorCode = modbus_lib_write_handler(start_addr.value + i + MB_ADDRESS_HOLDING_REGISTER_OFFSET, value.value);
                if (g_modbus_lib_exception_occurred){
                    g_modbus_lib_exception_occurred = 0;
                    return;
                }
